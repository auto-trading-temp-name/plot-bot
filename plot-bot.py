...
algorithms = ['bollinger_bands', 'rsi', 'macd', 'custom_bollinger_rsi', 'price']
...
async def plot(channel, plot_url):
	for algorithm in algorithms:
		...
		for interval in intervals:
			...
			if svg_tag:
				png_bytes = cairosvg.svg2png(bytestring=svg_tag.encode('utf-8'))
...
async def start_loop(ctx):
	if ctx.channel.id == int(CHANNEL_ID):
		...
		while True:
			...
			await asyncio.sleep(5 * 60 * 60) #Adjust time , + dont make it speceficly work with intervals and update each time they update not necessary 
